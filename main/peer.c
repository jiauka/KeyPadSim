...
static int peer_dsc_disced(uint16_t conn_handle, const struct ble_gatt_error* error, uint16_t chr_val_handle, const struct ble_gatt_dsc* dsc, void* arg);
...
static int peer_chr_disced(uint16_t conn_handle, const struct ble_gatt_error* error, const struct ble_gatt_chr* chr, void* arg)
{
  ...
  peer = arg;
  assert(peer->conn_handle == conn_handle);
  //  MODLOG_DFLT(INFO, "peer_chr_disced %04x",chr->uuid.u16.value);
  switch (error->status) {
    case 0:
